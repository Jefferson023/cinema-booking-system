...
public class UserSceneController {
	...
	void initialize() throws IOException {
		...
		File file2 = new File(path + "player-04.png");
		Image img2 = SwingFXUtils.toFXImage(ImageIO.read(file2), null);
	...
	public void playStopMusic(MouseEvent e) {
		 ...
		 * lang-illegalstateexception-not-on-fx-application-thread Avoid throwing
		 * IllegalStateException by running from a non-JavaFX thread. Beautiful lambda
		 * expression version.
		 */
		Platform.runLater(() -> {
	...
	protected void changeImage() throws IOException {
		...
		try {
			...
			if (selectedImage == null)
				...
				Alert alert = new Alert(AlertType.WARNING, "Please upload an image in PNG or JPG format!",
						ButtonType.OK);
				alert.showAndWait();
	...
	/**
	 * This method allows the employee to export a list of films for which any kind
	 * of activity has been perfomed such as booking a seat or delete a seat.
	 * 
	 * @param ActionEvent e
	 * @throws IOException
	 */
	...
	void exportFilmList(ActionEvent e) throws IOException {
		...
		PrintWriter pw = new PrintWriter(new FileOutputStream(new File(selectedDirectory.toPath() + "/bookings.csv")));
		pw.close();

		// creating the printwriter using the append option now
		pw = new PrintWriter(new FileOutputStream(new File(selectedDirectory.toPath() + "/bookings.csv"), true));

		// mapping film titles to a set of strings representing screening dates and
		// times
		...
		for (String filmTitle : map.keySet()) {
			...
			for (String dateAndTime : setOfDatesAndTimes) {
				...
				for (BookingHistoryItem booking : Main.getBookingList()) {
					...
					if (booking.getFilm().equals(filmTitle)
							&& ("date: " + booking.getDate() + ", time: " + booking.getTime()).equals(dateAndTime)
				...
				pw.append("title: " + filmTitle + ", " + dateAndTime + ", booked seats: "
						+ numberOfBookingsAtSpecificDateAndTime + ", available seats: "
						+ (18 - numberOfBookingsAtSpecificDateAndTime) + "\n");
			}
		}
